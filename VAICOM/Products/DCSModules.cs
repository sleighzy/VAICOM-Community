    ...
    namespace Products
    {
        ...
        public static class DCSmodules
        {
            public static DCSmodule findmodulebyid(string id)
            {
                ...
                {
                    if (id.Equals(entry.Value.Id, StringComparison.OrdinalIgnoreCase))
                    {
                        result = entry.Value;
                        break;
                    }
            ...
            public static Dictionary<string, DCSmodule> LookupTable = new Dictionary<string, DCSmodule>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                {"F-5E-3",    new DCSmodule() { Id ="F-5E-3",Name = "F-5E-3",    Alias = "Tiger II",    ProOnly = true,  IsFC = false, ApxWpn = true, ApxDir = false,  IsHelo = false, Flightmenu = DcsClient.iCommandsequences.showflight1 ,Singlehotkey = true,  Havedial = false, Theme = "Fallon",   SpeechAlias = "F Five E"  } }, // Includes F-5E Remastered
                ...
                {"MiG-29 Fulcrum", new DCSmodule() { Id ="MiG-29 Fulcrum",Name = "MiG-29A",Alias = "Fulcrum", ProOnly = true,  IsFC = false, ApxWpn = true, ApxDir = false,  IsHelo = false, Flightmenu = DcsClient.iCommandsequences.showflight1 ,Singlehotkey = false , Havedial = true, Theme = "Russia",  SpeechAlias = "Fulcrum" } },
